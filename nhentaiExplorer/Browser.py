...
class Browser(qtw.QGroupBox):
    ...
    def browser(self):
        self.browser_pages()
        self.browser_pages_widget.setStyleSheet(
            "#browser_pages_widget {border-top: 1px solid white}"
        )
        ...
        browser_horizontal_scrollbar = qtw.QScrollBar(
            objectName="browser_scrollbar_horizontal"
        )
        browser_vertical_scrollbar = qtw.QScrollBar(
            objectName="browser_scrollbar_vertical"
        )
    ...
    def browser_pages(self):
        ...
        self.left_btn = QPushButton(
            "<",
            clicked=lambda: self.prev_next_btn_clicked(mode="<"),
            objectName="EXP_PushButton",
        )
        ...
        self.page_number_input = qtw.QLineEdit(
            "0", alignment=qtc.Qt.AlignCenter, objectName="EXP_LineEdit"
        )
        self.page_number_input.setEnabled(False)
        self.page_number_input.returnPressed.connect(
            lambda: self.update_browser_page_number(mode="returnPressed")
        )
        self.page_number_input.textChanged.connect(check_int)
        self.page_number_label = qtw.QLabel(
            f"/ {self.max_page_numbers}", alignment=qtc.Qt.AlignCenter
        )
    ...
    def get_items(self, mode=None):
        temp = self.nhen_DBB.sqlite_select(
            get="rowid",
            order_by="ids",
            order_in="ASC",
        ...
        results = self.nhen_DBB.sqlite_select(
            get="*",
            order_by="ids",
            order_in="ASC",
            table="nhentaiLibrary",
    ...
    def create_items(self, gallery_data_dict_list):
        ...
        self.browser_items = [
            BrowserItemWidget() for i in range(len(gallery_data_dict_list))
        ]
        ...
        for index, gallery_data_dict in enumerate(gallery_data_dict_list):
            ...
            browser_item.BIW_hover_signal.connect(
                lambda mode, index: self.hover_event(mode, index)
            )
    ...
    def selection_changed(self, location=None, index=0):
        ...
        self.browser_items[index].layout().itemAt(0).widget().setStyleSheet(
            "background: #363636;"
        )
        self.browser_items[index].layout().itemAt(1).widget().setStyleSheet(
            "background: #363636;"
        )

        self.browser_items[index].setSelected(True)
        self.browser_items[index].layout().itemAt(1).widget().verticalScrollBar().show()
        self.browser_items[index].layout().itemAt(
            1
    ...
    def create_metadata(self, gallery_data_dict):
        ...
        title_id = qtw.QLabel(
            f"{gallery_data_dict['title']} ({gallery_data_dict['id']})"
        )
    ...
    def paintEvent(self, event):
        ...
        if (
            ...
            and self.current_page_number + 1 > self.max_page_numbers
        ):
            self.change_btn_state(False, False, False, False)
        if (
            not self.current_page_number - 1 <= 0
            and not self.current_page_number + 1 > self.max_page_numbers
        ):
    ...
    def hover_event(self, mode, index):
        ...
        if mode == 0:
            ...
            self.browser_items[index].layout().itemAt(
                1
            ).widget().horizontalScrollBar().hide()
...
class Worker(qtc.QObject):
    ...
    def resize_image(self, location, index):
        ...
        thumbnail_pixmap = thumbnail_pixmap.scaled(
            150, 200, qtc.Qt.KeepAspectRatio, qtc.Qt.SmoothTransformation
        )
