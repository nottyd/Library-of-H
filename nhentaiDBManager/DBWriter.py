...
class nhentaiLibrary:
    def create_database(self) -> None:
        self.c.execute(
            """ CREATE TABLE IF NOT EXISTS nhentaiLibrary (
                    ids INTEGER UNIQUE,
                    titles TEXT,
                    artists TEXT,
                    groups TEXT,
                ...
                if file.startswith("metadata.csv"):
                    with open(os.path.join(root, file), encoding="utf-8") as csv_file:
                        ...
                        self.data_dict.append(
                            {col["tag"]: col["tag_values"] for col in csv_file}
                        )
    ...
    def update_database(self, item, root) -> None:
        ...
        with self.conn:
            ...
            try:
                c.execute(
                    """INSERT INTO nhentaiLibrary (ids, titles, artists, groups, parodies, characters, languages, categories, pages, upload_date, tags, location)
                        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""",
                    (
                        int(item["code"]),
                        item["title"],
...
class DownloadedAndDuplicates:
    ...
    def set_database(
        self, database_location, database_filename="nhentaiDatabase.db"
    ) -> None:
        self.database_location = database_location
        self.database_filename = database_filename
        if not self.database_location.endswith(".db"):
    ...
    def update_database(self, downloaded_code, duplicate_codes) -> None:
        ...
        with self.conn:
            for duplicate_code in duplicate_codes:
                try:
                    c.execute(
                        f"""INSERT INTO duplicates (downloaded_id, duplicate_id) VALUES (?, ?)""",
                        (int(downloaded_code), int(duplicate_code)),
                    )
                except BaseException as e:
                    if "UNIQUE constraint failed" in " ".join(e.args):
                        pass
    ...
    def update_database(self, collection_codes) -> None:
        ...
        with self.conn:
            for collection_code in collection_codes:
                try:
                    c.execute(
                        f"""INSERT INTO collections (collection_id) VALUES ({int(collection_code)})"""
                    )
                except BaseException as e:
                    if "UNIQUE constraint failed" in " ".join(e.args):
                        pass
