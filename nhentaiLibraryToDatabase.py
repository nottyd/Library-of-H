...
parser = argparse.ArgumentParser(
    description="Create a database with provided nhentai Library. The database file will be created in accordance to config.ini"
)
parser.add_argument(
    "library_root_directory",
    type=validate_library_directory,
    help="Path to nhentai library directory.",
)
...
def library_to_database():
    ...
    if Path(f"{config.databaselocation}").exists():
        nhen_DBW_l.set_database(
            config.databaselocation, metadata_location=args.library_root_directory
        )
    else:
        ...
        nhen_DBW_l.set_database(
            config.default_databaselocation,
            metadata_location=args.library_root_directory,
        )
    nhen_DBW_l.set_database(
