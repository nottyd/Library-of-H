...
class MetadataHandler:
    def __init__(self, gallery_code, gallery_folder=None, config=None) -> None:
        try:
            ...
            if "metadata.csv" not in os.listdir():
                ...
                raise Exception(
                    f"metadata.csv already exists. Location: {os.path.abspath('.')}"
                )
    ...
    def database_writer(self, all_=False) -> None:
        ...
        if os.path.isdir(f"{self.config.databaselocation}"):
            nhen_DBW_l.set_database(
                self.config.databaselocation, metadata_location=os.path.abspath(".")
            )
        else:
            Helper.log_and_print(
                error_family="CONFError",
                error_type="databaselocation_load_error",
                cont_default=True,
            )
            nhen_DBW_l.set_database(
                self.config.default_databaselocation,
                metadata_location=os.path.abspath("."),
            )
    ...
    def writer(self, tags_dict_list) -> None:
        ...
        with open("metadata.csv", "w", encoding="utf-8") as metadata_file:
            csv_writer = csv.DictWriter(
                metadata_file, fieldnames=fieldnames, delimiter=":"
            )
    ...
    def parodies_getter(self, all_=False) -> Union[None, list, str]:
        data = [
            parody.get("href")[8:-1]
            for parody in self.soup.find_all("a", class_="tag")
            if parody.get("href").startswith("/parody/")
        ]
        if len(data) == 1:
            data = data[0]
        elif len(data) <= 0:
    ...
    def characters_getter(self, all_=False) -> Union[None, list, str]:
        data = [
            character.get("href")[11:-1]
            for character in self.soup.find_all("a", class_="tag")
            if character.get("href").startswith("/character/")
        ]
        if len(data) <= 0:
            data = "None"
        if all_ == True:
    ...
    def tags_getter(self, all_=False) -> Union[None, list, str]:
        data = [
            tag.get("href")[5:-1]
            for tag in self.soup.find_all("a", class_="tag")
            if tag.get("href").startswith("/tag/")
        ]
        if len(data) <= 0:
            data = "None"
        if all_ == True:
    ...
    def artists_getter(self, all_=False) -> Union[None, str]:
        data = [
            artist.get("href")[8:-1]
            for artist in self.soup.find_all("a", class_="tag")
            if artist.get("href").startswith("/artist/")
        ]
        if len(data) <= 0:
            data = "None"
        if len(data) > 5:
    ...
    def groups_getter(self, all_=False) -> Union[None, str]:
        data = [
            group.get("href")[7:-1]
            for group in self.soup.find_all("a", class_="tag")
            if group.get("href").startswith("/group/")
        ]
        if len(data) <= 0:
            data = "None"
        if all_ == True:
    ...
    def languages_getter(self, all_=False) -> Union[None, list, str]:
        data = [
            language.get("href")[10:-1]
            for language in self.soup.find_all("a", class_="tag")
            if language.get("href").startswith("/language/")
        ]
        if len(data) <= 0:
            data = "None"
        if all_ == True:
    ...
    def categories_getter(self, all_=False) -> Union[None, list, str]:
        data = [
            category.get("href")[10:-1]
            for category in self.soup.find_all("a", class_="tag")
            if category.get("href").startswith("/category/")
        ]
        if len(data) <= 0:
            data = "None"
        if all_ == True:
    ...
    def pages_getter(self, all_=False) -> Union[None, str]:
        data = [
            str(page.string)
            for page in self.soup.find_all("a", class_="tag")
            if page.get("href").startswith("/search/?q=pages")
        ]
        try:
            data = int(data[0])
        except Exception:
