...
class DownloadByGalleries:
    ...
    def download_by_galleries(self, gallery_folder=None) -> None:
        for index, gallery_code in enumerate(self.gallery_codes, start=1):
            ...
            if not response:
                GalleriesDownloader.galleries_downloader(gallery_codes=(gallery_code,), save_dest=self.save_dest, config=self.config)
            else:
    ...
    def handle_errors(self):
        if len(StaticVariables.name_too_long) > 0:
            for gallery_code, gallery_folder in StaticVariables.name_too_long:
                ...
                self.download_by_galleries(config=self.config, gallery_folder=gallery_folder)
