...
def downloader(image_links, save_dest, folder, config) -> None:
    ...
    for index, image_link in enumerate(image_links, start=1):
        while not done:
            ...
            if image_links[0] == image_link:
                Helper.print_bar_progress(
                    total=len_image_links, progress=0, msg="Downloaded pages"
                )
                Helper.set_console_title(download_progress=f"[{0} / {len_image_links}]")
            if already_downloaded != 0:
                Helper.print_bar_progress(
                    total=len_image_links,
                    progress=already_downloaded + (index - already_downloaded),
                    msg="Downloaded pages",
                )
                Helper.set_console_title(
                    download_progress=f"[{already_downloaded+(index-already_downloaded)} / {len_image_links}]"
                )
            ...
            try:
                ...
                if "[chinese]" in folder.lower():
                    ...
                    gallery_code = folder[
                        re.search("\([0-9]+\)$", folder)
                        .start() : re.search("\([0-9]+\)$", folder)
                        .end()
                    ]
        ...
        image_name = image_link[
            re.search("[0-9]+.[a-z]+", image_link)
            .start() : re.search("[0-9]+.[a-z]+", image_link)
            .end()
        ]
        ...
        if not config.overwrite:
            if not os.path.exists(image_name):
                _write_to_disk(
                    ...
                    len_image_links=len_image_links,
                    index=index,
                )
            else:
